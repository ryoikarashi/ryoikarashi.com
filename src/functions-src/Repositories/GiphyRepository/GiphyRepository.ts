...
export class GiphyRepository implements IGiphyRepository {
    ...
    async getRandom(): Promise<Giphy> {
        const limit = Number(process.env.GIPHY_SEARCH_LIMIT ?? this._fallbackSearchLimit);
        const { data } = await this._client.search(process.env.GIPHY_SEARCH_TERM ?? this._fallbackSearchTerm, {
            limit,
        });
