...
describe('Test TokenService', () => {
  describe('getAccessAndRefreshToken', () => {
    it('returns an existing token', async () => {
      ...
      const mockGetTokenByAuthorizationCode = jest
        .fn()
        .mockResolvedValueOnce(newToken);
      jest
        .spyOn(MockTokenRepositoryInstance, 'getFirstToken')
        .mockImplementationOnce(mockGetFirstToken);
      jest
        .spyOn(MockTokenRepositoryInstance, 'getTokenByAuthorizationCode')
        .mockImplementationOnce(mockGetTokenByAuthorizationCode);
    ...
    it('returns a newly issued token', async () => {
      ...
      jest
        .spyOn(
          MockTokenRepositoryInstance,
          'storeAccessTokenAndMaybeRefreshToken'
        )
        .mockImplementationOnce(mockStoreAccessTokenAndMaybeRefreshToken);

      await expect(service.getAccessAndRefreshToken()).resolves.toEqual(
        newToken
      );
      expect(mockGetFirstToken).toHaveBeenCalledTimes(1);
      expect(mockGetTokenByAuthorizationCode).toHaveBeenCalledTimes(1);
      expect(mockGetTokenByAuthorizationCode).toHaveBeenCalledWith(
        axios,
        oauthConfig
      );
      expect(mockStoreAccessTokenAndMaybeRefreshToken).toHaveBeenCalledTimes(1);
      expect(mockStoreAccessTokenAndMaybeRefreshToken).toHaveBeenCalledWith(
        newToken
      );
    });
  ...
  describe('refreshAccessToken', () => {
    it('it returns a new token', async () => {
      ...
      jest
        ...
        .mockImplementationOnce(mockGetFirstToken);
      jest
        .spyOn(MockTokenRepositoryInstance, 'refreshToken')
